...
android {
    ...
    defaultConfig {
        ...
        // ٢٠٢٣ / ١.٨.١ — توافقُ الأجهزة يعود: أُزيل حصرُ arm64 (كان يُسقط ٩٬٤٣٢ جهازاً
        // عند الرفع) وأُزيلت مكتبة WebRTC الميّتة بعد إلغاء المكالمات — فلا مكتبة
        // أصليّة ثقيلة في الحزمة أصلاً، والحزمة أخفّ ممّا كانت وهي تدعم كلّ المعالجات.
        // وأُزيلت أذونات المكالمات الميتة (شاشة كاملة، توجيه الصوت، ميكروفون الخدمة).
        // ٢٠٢٤ / ١.٨.٢ — بطاقات الإحصاء تتقاسم عرض الصفّ (كان الرقم يتقطّع على
        // شاشة ٣٢٠dp)؛ ورقمٌ جديد لأن ٢٠٢٣ نُشر قبل هذا الإصلاح.
        // ٢٠٢٥ / ١.٨.٣ — لا انهيار عند انقطاع الشبكة: تدفّق «الدروس المميّزة» وحده
        // كان بلا `runCatching`، فأيّ UnknownHostException أثناء الاستطلاع تُرمى
        // داخل `collectAsState` فتُغلق اللوحة — أكثر أعطالها في Play Vitals
        // (8 تقارير من مشرفَين، حتى ٢٠٢٤). تبقى الآن آخر قائمة ويُعاد في الدورة التالية.
        // ٢٠٢٦ / ١.٨.٣ — المحتوى نفسه؛ ٢٠٢٥ لم يُبنَ قطّ لأنّ بوّابة النشر كانت تقرأ
        // «versionCode=2001» من تعليقٍ أعلاه بدل سطر الإسناد، فتقف عند كلّ رفع.
        versionCode = 2026
        // ⛔⛔ **لا `abiFilters` هنا أبداً** (درسٌ بثمنه 2026-09-12): حصرُ الحزمة في
        //    arm64 وحدَه جعل Play يرفض الرفع — «لم يعد هذا الإصدار متوافقاً مع 9,432
        //    جهازاً كانت متوافقة مع الإصدار السابق»، ومن ثبّتها منهم لا تصله تحديثات.
        //    والحصرُ لم يكن يوفّر على المستخدم شيئاً أصلاً: **Play يقسّم الـAAB بحسب
        //    المعالج** فلا يُنزّل الجهازُ إلا مكتبةَ معالجه هو؛ الأربعون ميغابايت كانت
        //    حجم ملفّ الرفع على المالك لا حجم التنزيل على المستخدم.
        //    والعلاجُ الصحيح للحجم هو حذفُ المكتبة الأصليّة نفسها: مكتبة WebRTC
        //    (١١٫٥ م.ب لكل معالج) بقيت مشحونة بعد إلغاء المكالمات — أُزيلت هنا.
        versionName = "1.8.3"
...
dependencies {
    ...
    // ⛔ لا WebRTC: المكالمات أُلغيت (2026-09-11) وبقيت مكتبتها تشحن
    //    `libjingle_peerconnection_so` ‏١١٫٥ م.ب لكل معالج بلا مستدعٍ واحد — حُذفت
    //    2026-09-12، ولم يبقَ في الحزمة إلا مكتبتان صغيرتان من AndroidX (١٧ ك.ب).
