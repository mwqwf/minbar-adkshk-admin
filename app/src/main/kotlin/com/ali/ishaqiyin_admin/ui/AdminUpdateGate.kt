...
fun AdminUpdateGate(isOwner: Boolean, content: @Composable () -> Unit) {
    ...
    LaunchedEffect(isOwner) {
        ...
        //    ⛔ ويُستثنى بناء debug: حزمته ورقمه مطابقان للإصدار (لا
        //    applicationIdSuffix)، فبلا هذا الحارس يُعلَن إصدارٌ لم يُنشر
        //    في المتجر بعدُ — وهي حادثة الإعلان الكاذب نفسها.
        if (isOwner &&
            !com.ali.ishaqiyin_admin.BuildConfig.DEBUG &&
            runCatching { repo.autoPublishOwnVersion() }.getOrDefault(false)
        ) {
            // ⚠️ نصّ «الجديد» **بيانات لا كود**: يُقرأ من حقل `message` في
            // `app_config/admin_android` — نفس الحقل الذي يكتبه المالك في
